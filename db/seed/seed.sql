...
INSERT INTO rules (domain,key,value,source) VALUES
 ...
 ('stack','credentials','project .env (gitignored), standard var names GOOGLE_API_KEY | GOOGLE_APPLICATION_CREDENTIALS+GOOGLE_CLOUD_PROJECT+GOOGLE_CLOUD_LOCATION | CLOUDFLARE_API_TOKEN+CLOUDFLARE_ACCOUNT_ID; never in db or commits','core'),
 ('stack','cloudflare_image_gen','sibling image route to the Gemini ladder, needs CLOUDFLARE_API_TOKEN+CLOUDFLARE_ACCOUNT_ID. Native @cf/... models (e.g. flux-2-klein-9b) via POST /accounts/{id}/ai/run/{model} are FREE and run on Cloudflare compute - but require multipart/form-data even for a text-only prompt, a JSON body 400s with a generic AiError no matter how correct it looks. BYOK-proxied models (openai/gpt-image-2) via POST /accounts/{id}/ai/run with {model,input} JSON support native transparent+webp output but are BILLED (402 Insufficient balance code 2021 on an unfunded gateway) - confirm funding before promising this route, never assume free just because it shares the /ai/run path with native models','core'),
...
INSERT INTO framework_map (path,kind,purpose,load_when,links) VALUES
 ...
 ('skills/structure-law/SKILL.md','skill','canonical file trees per stack + intake questions + API versioning norms (/v1 path default vs header-based)','scaffolding, moving files, new projects, designing API routes','nextjs,fastapi'),
